...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_path", type=str, default="../data/er_magellan/Structured/Beer/"
    )
    ...
    if mata and matb:
        pairs = blocked_matmul(
            mata, matb, threshold=hp.threshold, k=hp.k, batch_size=hp.batch_size
        )
        dump_pairs(
            os.path.join(hp.input_path, hp.output_fn), entries_a, entries_b, pairs
        )
